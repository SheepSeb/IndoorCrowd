...
def run_scene(
    ...
    if args.min_track_len > 1:
        ...
        mot_rows = [
            ",".join([r.split(",")[0], str(remap[int(r.split(",")[1])]), *r.split(",")[2:]])
            for r in mot_rows
        ]
...
def main() -> int:
    ...
    parser.add_argument("--output-dir",     type=Path, default=Path("dataset/MOT_labels_sam3_5fps"))
    ...
    print(f"Part size   : {args.part_size} frames  |  min-track-len={args.min_track_len}")
