#!/usr/bin/env python3
"""EfficientGroundedSAM (Grounding DINO + EdgeSAM) → COCO JSON for dataset/main.
...
Output layout
-------------
  dataset/main/labels_efficient_grounded_sam/
    {scene}/
      {recording}.json
    timing.json
...
Usage
-----
  uv run python labelling/autolabel_efficient_grounded_sam_main.py

  # Specific scenes
  uv run python labelling/autolabel_efficient_grounded_sam_main.py --scenes acs_ec acs_eg
...
def ensure_edgesam_segment_anything_available() -> None:
    """Prefer EdgeSAM's bundled segment_anything so registry contains 'edge_sam'."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="EfficientGroundedSAM (Grounding DINO + EdgeSAM) → COCO JSON for dataset/main"
    )
    parser.add_argument("--raw-frames-dir", type=Path,
                        default=Path("dataset/main/raw_frames_5fps"))
    parser.add_argument("--output-dir",     type=Path,
                        default=Path("dataset/main/labels_efficient_grounded_sam"))
    ...
    print("Loading EfficientGroundedSAM (GroundedEdgeSAM) ...")
    ...
    print(f"Model       : EfficientGroundedSAM  (load: {model_load_s:.1f}s)")
