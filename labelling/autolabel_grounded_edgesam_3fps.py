"""Auto-label 3fps frames with Efficient Grounded SAM (GroundedEdgeSAM).
...
Default layout:
    ...
    output: dataset/labels_efficient_grounded_sam_3fps/{split}/{scene}.json
"""
...
def ensure_transformers_compatible() -> tuple[bool, str]:
    """Grounded*SAM wrappers currently depend on a transformers<5 API."""
    try:
        import transformers
    except Exception:
    ...
    if _major_version(version) >= 5:
        return (
            ...
            (
                ...
                "autodistill grounded sam wrappers currently need transformers<5.\n"
                "Fix with:\n"
...
def ensure_pip_command() -> str:
    """Ensure a `pip` command is available for autodistill subprocess calls.

    autodistill_grounded_edgesam internally runs `subprocess.run(["pip", ...])`.
    In `uv run` environments, `pip` might not be on PATH, so we create a tiny
    shim that forwards to `python -m pip`.
    """
...
def ensure_edgesam_segment_anything_available() -> None:
    """Prefer EdgeSAM's bundled `segment_anything` implementation.

    `autodistill_grounded_edgesam` expects `sam_model_registry["edge_sam"]`.
    The stock `segment_anything` package does not always expose that key.
    """
    edge_repo = Path.home() / ".cache" / "autodistill" / "EdgeSAM"
    edge_segment_anything = edge_repo / "segment_anything"
    if not edge_segment_anything.exists():
        return

    if str(edge_repo) not in sys.path:
        sys.path.insert(0, str(edge_repo))

    # If segment_anything was imported earlier in this process, reload it from
    # EdgeSAM path so registry contains the "edge_sam" key.
    if "segment_anything" in sys.modules:
        del sys.modules["segment_anything"]
    if "segment_anything.build_sam" in sys.modules:
        del sys.modules["segment_anything.build_sam"]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Auto-label frames using GroundedEdgeSAM into COCO JSON."
    )
    parser.add_argument(
        "--raw-frames-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Output COCO labels directory (default: dataset/labels_efficient_grounded_sam_3fps)",
    )
    parser.add_argument(
        "--splits",
        nargs="+",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.35,
        help="GroundedEdgeSAM box threshold.",
    )
    parser.add_argument(
        "--text-threshold",
        type=float,
        default=0.25,
        help="GroundedEdgeSAM text threshold.",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
...
def main() -> int:
    ...
    try:
        ...
        print(
            "Missing dependency: autodistill-grounded-edgesam.\n"
            "Install with: pip install autodistill-grounded-edgesam"
        )
        return 1

    class_names = list(OrderedDict.fromkeys(ontology_pairs.values()))
    pip_cmd = ensure_pip_command()
    ensure_edgesam_segment_anything_available()
    print("Loading GroundedEdgeSAM ...")
    print(f"transformers: {transformers_info}")
    print(f"pip command: {pip_cmd}")
    model = GroundedEdgeSAM(
