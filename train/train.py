"""
Train ultralytics YOLO / RT-DETR models on the golden_frames_3fps_mid dataset.
...
--------
# detection
uv run python train/golden_mid/train.py --task detect --model yolo11n yolo26n rtdetr-l

# segmentation (yolo/rt-detr models that don't support seg are skipped automatically)
uv run python train/golden_mid/train.py --task segment --model yolo11n yolo26n

# multiple models in sequence
uv run python train/golden_mid/train.py --task detect --model yolo11n yolo11s yolo26n rtdetr-l
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train YOLO/RT-DETR on golden_mid dataset")
    parser.add_argument(
        "--task",
        choices=["detect", "segment"],
        default="detect",
        help="detect or segment (default: detect)",
    )
    parser.add_argument(
        "--model",
        nargs="+",
